...
function SlashCmdList.AUX(command)
    ...
    elseif arguments[1] == 'populate' and arguments[2] == 'wdb' then
		...
		aux.print(L['Usage'] .. ': ')
		aux.print('- scale [' .. aux.color.blue(aux_scale) .. '|cffffff7f]|r: ' .. L['AUX_CMD_1'])
		aux.print('- ignore owner [' .. status(aux_ignore_owner) .. '|cffffff7f]|r: ' .. L['AUX_CMD_2'])
		aux.print('- post bid [' .. status(aux_post_bid) .. '|cffffff7f]|r: ' .. L['AUX_CMD_3'])
		aux.print('- crafting cost [' .. status(aux_crafting_cost) .. '|cffffff7f]|r: ' .. L['AUX_CMD_4'])
