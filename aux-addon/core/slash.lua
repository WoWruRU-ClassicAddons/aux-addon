...
function status(enabled)
	return (enabled and aux.color.green..L['on'] or aux.color.red..L['off'])
end
...
function SlashCmdList.AUX(command)
    ...
    elseif arguments[1] == 'post' and arguments[2] == 'duration' and  T.map('2', post.DURATION_2, '8', post.DURATION_8, '24', post.DURATION_24)[arguments[3]] then
        ...
        aux.print('post duration ' .. aux.color.blue(aux.account_data.post_duration / 60 .. L['h']))
    ...
    elseif arguments[1] == 'populate' and arguments[2] == 'wdb' then
		...
		aux.print(L['Usage'] .. ': ')
		aux.print('- scale [' .. aux.color.blue(aux.account_data.scale)                                     .. '|cffffff7f]|r: ' .. L['AUX_CMD_1'])
		aux.print('- ignore owner [' .. status(aux.account_data.ignore_owner)                               .. '|cffffff7f]|r: ' .. L['AUX_CMD_2'])
		aux.print('- post bid [' .. status(aux.account_data.post_bid)                                       .. '|cffffff7f]|r: ' .. L['AUX_CMD_3'])
		aux.print('- post duration [' .. aux.color.blue(aux.account_data.post_duration / 60 .. L['h'])      .. '|cffffff7f]|r: ' .. L['AUX_CMD_4'])
		aux.print('- crafting cost [' .. status(aux.account_data.crafting_cost)                             .. '|cffffff7f]|r: ' .. L['AUX_CMD_5'])
		aux.print('- tooltip value [' .. status(tooltip_settings.value)                                     .. '|cffffff7f]|r: ' .. L['AUX_CMD_6'])
		aux.print('- tooltip daily [' .. status(tooltip_settings.daily)                                     .. '|cffffff7f]|r: ' .. L['AUX_CMD_7'])
		aux.print('- tooltip merchant buy [' .. status(tooltip_settings.merchant_buy)                       .. '|cffffff7f]|r: ' .. L['AUX_CMD_8'])
		aux.print('- tooltip merchant sell [' .. status(tooltip_settings.merchant_sell)                     .. '|cffffff7f]|r: ' .. L['AUX_CMD_9'])
		aux.print('- tooltip disenchant value [' .. status(tooltip_settings.disenchant_value)               .. '|cffffff7f]|r: ' .. L['AUX_CMD_10'])
		aux.print('- tooltip disenchant distribution [' .. status(tooltip_settings.disenchant_distribution) .. '|cffffff7f]|r: ' .. L['AUX_CMD_11'])
		aux.print('- clear item cache' .. '|r: ' .. L['AUX_CMD_12'])
		aux.print('- populate wdb' .. '|r: ' .. L['AUX_CMD_13'])
    end
end