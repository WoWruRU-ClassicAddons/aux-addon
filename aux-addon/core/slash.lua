...
function SlashCmdList.AUX(command)
    ...
    elseif arguments[1] == 'populate' and arguments[2] == 'wdb' then
		...
		aux.print('- post duration [' .. aux.color.blue(aux.account_data.post_duration / 60 .. 'h') .. ']')
		aux.print('- crafting cost [' .. status(aux.account_data.crafting_cost) .. '|cffffff7f]|r: ' .. L['AUX_CMD_4'])
		aux.print('- tooltip value [' .. status(tooltip_settings.value) .. '|cffffff7f]|r: ' .. L['AUX_CMD_5'])
		aux.print('- tooltip daily [' .. status(tooltip_settings.daily) .. '|cffffff7f]|r: ' .. L['AUX_CMD_6'])
		aux.print('- tooltip merchant buy [' .. status(tooltip_settings.merchant_buy) .. '|cffffff7f]|r: ' .. L['AUX_CMD_7'])
		aux.print('- tooltip merchant sell [' .. status(tooltip_settings.merchant_sell) .. '|cffffff7f]|r: ' .. L['AUX_CMD_8'])
		aux.print('- tooltip disenchant value [' .. status(tooltip_settings.disenchant_value) .. '|cffffff7f]|r: ' .. L['AUX_CMD_9'])
		aux.print('- tooltip disenchant distribution [' .. status(tooltip_settings.disenchant_distribution) .. '|cffffff7f]|r: ' .. L['AUX_CMD_10'])
		aux.print('- clear item cache' .. '|r: ' .. L['AUX_CMD_11'])
		aux.print('- populate wdb' .. '|r: ' .. L['AUX_CMD_12'])
    end
end